...
export function createBlankSpan() {
  return baseSpan.cloneNode(false) as HTMLSpanElement
}

function getSpanWithClass(className: string) {
  const span = createBlankSpan()
  span.className = className
  return span
}

function getSpanWithBoth(innerText: string, className: string) {
  const span = createBlankSpan()
  span.className = className
